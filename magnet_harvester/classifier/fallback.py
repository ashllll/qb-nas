...
LOCAL_RULES: List[Tuple[str, str]] = [
    ...
    # ── 体育（优先于电影年份）──
    (
        r"\b(ufc|mma|boxing|wrestling|nba|nfl|nhl|mlb|f1\b|formula\s*one|motogp|premier\s*league|laliga|serie\s*a)\b",
        "其他",
    ),
    # ── 教程/学习 ──
    ...
    (
        r"\b(lolihouse|sakurato|hysub|littlebaka|reinforce|vcb-studio|mabors|ane|subsplease|dmhy|moozzi2|kamigami|jsum)\b",
        "动漫",
    ),
    # 剧集格式 + 动漫源 → 动漫（防止被电视剧规则抢先匹配）
    (
        r"\.s\d{2,3}.*\.(?:cr|crunchyroll|b-global|baha|sentai|hidive|funimation|varyg|asw|scy|ember|kaleido|doki|horriblesubs|erai-raws)\b",
        "动漫",
    ),
    # ── 电视剧/剧集 ──
    ...
    # ── 软件 ──
    (
        r"setup\.exe|installer|\bcrack\b|keygen|patch\b|serial\s*key|激活|注册机|绿色版|便携版|单文件版",
        "软件",
    ),
    (
        r"\b(adobe|autodesk|microsoft|vmware|oracle|photoshop|illustrator|after\s*effects|premiere|windows[\s._]\d+|win[\s._]?\d+)",
        "软件",
    ),
...
COMPILED_LOCAL_RULES: list[tuple[re.Pattern, str]] = [
...
def classify_local(name: str) -> str:
    """使用本地规则分类，按优先级匹配"""
    for pattern, cat in COMPILED_LOCAL_RULES:
